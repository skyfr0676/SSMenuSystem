...
namespace ServerSpecificSyncer.Features.Wrappers
{
    public class Button : SSButton, ISetting
    {
        ...
        public Button(int? id, string label, string buttonText, Action<ReferenceHub, SSButton> onClick,
            float? holdTimeSeconds = null, string hint = null) : base(id, label, buttonText, holdTimeSeconds, hint)
        {
            Action = onClick;
