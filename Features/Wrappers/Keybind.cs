// -----------------------------------------------------------------------
// <copyright file="Keybind.cs" company="Skyfr0676 and Redforce04">
...
/// <summary>
/// Initialize a new instance of wrapper for <see cref="SSKeybindSetting"/> with the <see cref="IsGlobal"/> addition (according to menu). This setting can allow player to bind a key (like cmd_bind) but for an action, and not a command. Because of pages, I added <see cref="IsGlobal"/> paameter, that determinate if the setting should be seen in all pages or not.
/// </summary>
public class Keybind : SSKeybindSetting, ISetting
{
    ...
    public Keybind(
        int? id,
        string label,
        Action<ReferenceHub, bool>? onUsed = null,
        KeyCode suggestedKey = KeyCode.None,
        bool preventInteractionOnGui = true,
        string? hint = null,
        bool isGlobal = true)
        : base(id + Increment, label, suggestedKey, preventInteractionOnGui, hint)
    {
        this.IsGlobal = isGlobal;
        this.Action = onUsed;
        this.Base = new SSKeybindSetting(id + Increment, label, suggestedKey, preventInteractionOnGui, hint);
    }

    /// <summary>
    /// Gets a value indicating whether the <see cref="SSKeybindSetting"/> would be shown and enabled on all pages or not.
    /// </summary>
    public bool IsGlobal { get; }

    /// <summary>
    /// Gets the method that will be executed when the value is updated. It's contains two parameters: <br></br><br></br>
