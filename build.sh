    ...
    for ((i = 1; i < $# + 1; i++)); do
        ...
        if [ "$arg" == "github" ]; then # push new version to github
            ...
            if [ -n "${!next_index}" ]; then
                echo "API Key found"
                API_KEY="${!next_index}"
                nuget push "./pack/SSMenuSystem-EXILED.$VERSION-EXILED.nupkg" -s https://api.nuget.org/v3/index.json -k "$API_KEY"
                nuget push "./pack/SSMenuSystem-NWAPI.$VERSION-NWAPI.nupkg" -s https://api.nuget.org/v3/index.json -k "$API_KEY"
            else
