// -----------------------------------------------------------------------
// <copyright file="EventHandler.cs" company="Skyfr0676 and Redforce04">
...
internal class EventHandler : CustomEventsHandler
{
    ...
    /// <param name="ss">The ServerSpecificSetting.</param>
    /// <exception cref="Exception">Thrown if the ServerSpecificSetting was not registered via this plugin.</exception>
    /// <exception cref="KeyNotFoundException">Thrown if the ServerSpecificSetting has an invalid id.</exception>
        ...
        catch (Exception e)
        {
            ...
            if (Plugin.Instance!.Config.ShowErrorToClient)
            {
                Utils.SendToPlayer(
                    hub,
                    null,
                    [new SSTextArea(-5, $"<color=red><b>{Plugin.Instance.Translation.ServerError}\n{((hub.serverRoles.RemoteAdmin || Plugin.Instance.Config.ShowFullErrorToClient) && Plugin.Instance.Config.ShowFullErrorToModerators ? e.ToString() : Plugin.Instance.Translation.NoPermission)}</b></color>", SSTextArea.FoldoutMode.CollapsedByDefault, Plugin.Instance.Translation.ServerError), new SSButton(-999, Plugin.Instance.Translation.ReloadButton.Label, Plugin.Instance.Translation.ReloadButton.ButtonText),]);
            }
        }
    }
